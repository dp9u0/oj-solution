...
[LeetCode Problem Description](https://leetcode.com/problems/maximum-rows-covered-by-columns/description/)
...
给定 m×n 二进制矩阵和整数 numSelect，选恰好 numSelect 列。一行被覆盖的条件：该行所有 1 所在列都被选中（或该行全 0）。返回最大覆盖行数。

## 解题思路

**位掩码枚举**

m, n ≤ 12，枚举所有 C(n, numSelect) 种选列方案。每行用位掩码表示哪些列有 1，检查 rowMask & ~selectMask == 0 即可。
