...
[LeetCode Problem Description](https://leetcode.com/problems/number-of-adjacent-elements-with-the-same-color/description/)
...
初始全0数组，每次查询将某位置设为某颜色，统计相邻同色对数。增量更新。

## 解题思路

维护全局 count。每次修改位置 idx 前，先减去 idx 与左右邻居的同色贡献；修改后再加上新的贡献。O(1) 每次查询，O(n+q) 总体。
