...
[LeetCode Problem Description](https://leetcode.com/problems/last-visited-integers/description/)
...
If -1is encountered, let k be the number of consecutive -1s seen so far (including the current -1),
...
给定数组 nums，正整数插入 seen 头部，-1 时根据连续 -1 的个数 k 从 seen 取第 k 个元素加入 ans（k 超出长度则取 -1）。返回 ans。

## 解题思路

模拟即可。维护 seen 数组和连续 -1 计数 k。遇到正整数插入头部并重置 k；遇到 -1 递增 k 并从 seen 取值。
