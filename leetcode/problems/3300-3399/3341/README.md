...
[LeetCode Problem Description](https://leetcode.com/problems/find-minimum-time-to-reach-last-room-i/description/)
...
n×m 网格，moveTime[i][j] 表示房间 (i,j) 最早可进入的时间。从 (0,0) 出发（t=0），每次移动到相邻房间需要 1 秒，但只能在房间开放后进入。求到达 (n-1, m-1) 的最短时间。

## 解题思路

**Dijkstra 最短路**

到达相邻房间 (nr, nc) 的时间 = max(当前时间, moveTime[nr][nc]) + 1。用优先队列的 Dijkstra 求从 (0,0) 到 (n-1,m-1) 的最短时间。
