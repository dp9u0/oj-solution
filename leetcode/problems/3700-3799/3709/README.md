...
[LeetCode Problem Description](https://leetcode.com/problems/design-exam-scores-tracker/description/)
...
设计考试分数追踪器。record(time, score) 按严格递增时间记录分数，totalScore(startTime, endTime) 返回 [startTime, endTime] 范围内所有分数之和。

## 解题思路

**前缀和 + 二分查找**

record 追加到有序数组并维护前缀和。totalScore 用二分找 [startTime, endTime] 的左右边界，用前缀和 O(1) 求区间和。每次操作 O(log n)。
