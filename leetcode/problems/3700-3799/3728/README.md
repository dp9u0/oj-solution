...
[LeetCode Problem Description](https://leetcode.com/problems/stable-subarrays-with-equal-boundary-and-interior-sum/description/)
...
给定数组capacity，统计满足以下条件的子数组[l..r]数量：长度>=3，首尾元素相等且等于中间元素之和。

## 解题思路

条件转化为：capacity[l]==capacity[r]且prefix[r]-prefix[l+1]==capacity[r]。即prefix[l+1]==prefix[r]-capacity[r]且capacity[l]==capacity[r]。用Map存储(prefix[j],capacity[j-1])的计数，遍历r时查询。
