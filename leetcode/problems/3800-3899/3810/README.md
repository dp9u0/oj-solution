...
[LeetCode Problem Description](https://leetcode.com/problems/minimum-operations-to-reach-target-array/description/)
...
给定两个长度为 n 的整数数组 nums 和 target。每次操作：选择值 x，找到所有值为 x 的极大连续段，将这些段替换为 target 对应值。求使 nums 变为 target 的最少操作次数。

## Approach

贪心扫描。维护一个 set 记录当前需要同时处理的 nums 值：
- 如果 nums[i] != target[i]，将 nums[i] 加入 set（如果之前不在 set 中则 ans++）
- 如果 nums[i] == target[i]，清空 set（因为连续性被打破）

这样 ans 就是需要多少层"同时存在"的不同值，即最少操作次数。
时间复杂度 O(n)，空间复杂度 O(n)。
