...
[LeetCode Problem Description](https://leetcode.com/problems/design-ride-sharing-system/description/)
...
设计拼车系统：管理乘客和司机的队列。addRider/addDriver 入队，matchDriverWithRider FIFO 匹配最早到的司机和乘客，cancelRider 取消未匹配的乘客。

## 解题思路

两个数组分别维护乘客和司机队列（FIFO），用 Set 辅助快速判断乘客是否存在以支持 cancel 操作。
