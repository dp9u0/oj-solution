...
[LeetCode Problem Description](https://leetcode.com/problems/count-subarrays-with-cost-less-than-or-equal-to-k/description/)
...
给定数组nums和整数k。子数组cost = (max - min) * length。求cost <= k的子数组个数。

## 解题思路

滑动窗口 + 单调双端队列。用两个单调队列分别维护窗口内最大值和最小值。窗口右端扩展时，如果cost超过k则收缩左端。对每个右端点，记录满足条件的最小左端点，统计子数组数量。
