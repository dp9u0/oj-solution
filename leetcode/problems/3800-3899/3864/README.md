...
[LeetCode Problem Description](https://leetcode.com/problems/minimum-cost-to-partition-a-binary-string/description/)
...
给定二进制字符串 s 和两个整数 encCost、flatCost。将字符串分成若干段，每段长度 L 含 X 个 '1'：若 X=0 花费 flatCost，否则花费 L*X*encCost。只能将偶数长度段等分为两段。求最小总花费。

## 解题思路

段大小只能从 n 不断折半直到奇数或 1，共 O(log n) 级，总段数 O(n)。自底向上 DP：从最小段开始计算 base cost，逐级合并时取 min(base, left+right)。用前缀和快速统计 1 的个数。整体 O(n) 时间。
