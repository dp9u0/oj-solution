...
给定长度 256 的 count 数组，count[k] 表示值 k 出现的次数。计算 [minimum, maximum, mean, median, mode]。

## 解题思路

遍历 count 数组：
- minimum：第一个 count > 0 的下标
- maximum：最后一个 count > 0 的下标
- mean：sum / total，其中 sum = Σ(i * count[i])
- median：找到排序后第 (total-1)/2 和 total/2 位置的值，取平均
- mode：count 最大的下标
