...
[LeetCode Problem Description](https://leetcode.com/problems/the-k-strongest-values-in-an-array/description/)
...
==
1 - 3
but 5 is stronger than 1 because 5 > 1.
...
给定数组arr和整数k。中位数m为排序后位置(n-1)/2的元素。"强度"定义为|arr[i]-m|，相等时值大的更强。返回最强的k个值。

## 解题思路

排序后找到中位数m。双指针从两端取，比较左右两端到中位数的距离，取较大的。
