...
[LeetCode Problem Description](https://leetcode.com/problems/design-sql/description/)
...
设计 SQL：建 n 张表（表名 + 列数）；`ins` 插入行（自增 id 从 1 起，删除不回退；列数不匹配或表不存在返回 false）；`rmv` 删除行；`sel` 查单元格（无效返回 `'<null>'`）；`exp` 导出全部行为 `id,v1,v2,...` 的字符串数组。

## 解题思路

每张表 = `{cols, rows: Map<id, row[]>, nextId}`：Map 保序导出，nextId 单调自增即可。各操作 O(1)/O(行数)。