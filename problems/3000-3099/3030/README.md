...
[LeetCode Problem Description](https://leetcode.com/problems/find-the-grid-of-region-average/description/)
...
Explanation:
There is only one 3 x 3 subgrid, while it does not have the condition on difference of adjacent pixels, for example, the difference between image[0][0] and image[1][0] is
5 - 8
= 3 > threshold = 1. None of them belong to any valid regions, so the result should be the same as image.
...
给定 m×n 灰度图像和阈值 threshold。找到所有满足相邻像素差 <= threshold 的 3×3 区域，计算每个区域的平均强度（向下取整）。每个像素的结果为其所属区域平均的均值（向下取整），不属于任何区域则保留原值。

## 解题思路

**暴力枚举 3×3 区域**

枚举所有可能的 3×3 子网格，检查相邻像素差是否 <= threshold。对有效区域记录向下取整的均值，最后汇总到每个像素。O(m*n) 时间。
