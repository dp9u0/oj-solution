...
[LeetCode Problem Description](https://leetcode.com/problems/count-paths-with-the-given-xor-value/description/)
...
给定 m*n 矩阵和整数 k，从(0,0)到(m-1,n-1)只能右移或下移，统计路径上所有元素 XOR 值等于 k 的路径数。

## 解题思路

**DP**：dp[j][x] = 到达(i,j)时 XOR 值为 x 的路径数。值域 <16，每步 XOR 转移。逐行更新，O(m*n*16)。
