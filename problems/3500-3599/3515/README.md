...
[LeetCode Problem Description](https://leetcode.com/problems/shortest-path-in-a-weighted-tree/description/)
...
给定带权树（根为1），支持两种操作：修改边权、查询根到某节点的最短路径距离。

## 解题思路

树上路径唯一，距离即为路径边权和。用Euler tour（DFS序）+ BIT（树状数组）实现子树范围加法。修改边权时，找到子节点，对其子树范围加delta。查询时返回初始距离+BIT点查询。O((n+q)·log n)。
