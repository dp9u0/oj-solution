...
[LeetCode Problem Description](https://leetcode.com/problems/minimum-weighted-subgraph-with-the-required-paths-ii/description/)
...
给定带权无向树和多组查询 [src1, src2, dest]，求最小权连通子树使 src1 和 src2 都能到达 dest。

## 解题思路

树上三点 {a,b,c} 的最小连通子树（Steiner树）权值 = (dist(a,b)+dist(a,c)+dist(b,c))/2，因为每条边恰被两条路径覆盖。用 LCA 二进制提升 O(nlogn) 预处理，每查询 O(logn)。
