...
[LeetCode Problem Description](https://leetcode.com/problems/minimum-discards-to-balance-inventory/description/)
...
给定数组arrivals、窗口大小w和限制m。每个物品到达时可保留或丢弃。任何w天窗口内每种类型的保留物品不超过m个。求最少丢弃数。

## 解题思路

贪心。对每种类型维护双端队列记录保留物品位置。处理每个物品时先移除窗口外的位置，若当前类型保留数<m则保留，否则必须丢弃。
