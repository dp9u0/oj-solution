...
[LeetCode Problem Description](https://leetcode.com/problems/minimum-cost-to-merge-sorted-lists/description/)
...
给定多个已排序数组 lists，每次选择两个合并，代价为 len(a)+len(b)+|median(a)-median(b)|。合并后插入新数组，直到只剩一个。求最小总代价。

## 解题思路

状压 DP。n <= 12，用 bitmask 表示集合。

1. 预处理每个子集 mask 的总长度和合并排序后的中位数
2. dp[mask] = 将 mask 中所有数组合并为一的最小代价
3. 转移：dp[mask] = min(dp[sub] + dp[mask^sub] + mergeCost(sub, mask^sub))，其中 sub 是 mask 的非空真子集，且 sub 包含 mask 的最低位（避免重复计数）
4. mergeCost = len1 + len2 + |median1 - median2|

时间复杂度 O(3^n + 2^n * L*logL)，其中 L 为总长度。
