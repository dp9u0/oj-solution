...
[LeetCode Problem Description](https://leetcode.com/problems/minimum-edge-toggles-on-a-tree/description/)
...
给定一棵树，每次操作选择一条边，翻转两端点的颜色（0↔1）。求将 start 变为 target 的最少操作对应的边编号（升序），不可能则返回 [-1]。

## 解题思路

**树上DFS + 奇偶性约束**

计算 diff[i] = start[i] XOR target[i]，即每个节点需要被翻转奇(1)偶(0)次。每条边的翻转同时影响两端点，因此翻转节点总数必为偶数（diff之和为奇则无解）。

以节点 0 为根，自底向上确定每条边：对非根节点 u，若其子树中已累积的翻转次数与 diff[u] 奇偶不同，则必须翻转 u 到父节点的边。最后检查根节点是否满足约束。O(n)。
