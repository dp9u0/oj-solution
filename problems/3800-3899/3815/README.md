...
[LeetCode Problem Description](https://leetcode.com/problems/design-auction-system/description/)
...
设计拍卖系统，支持添加/更新/删除出价，查询某物品最高出价者（相同金额取最大userId，无出价返回-1）。

## 解题思路

用嵌套Map存储：Map<itemId, Map<userId, bidAmount>>。添加/更新/删除直接操作Map。查询时遍历该物品所有出价，按(amount, userId)找最大值。
