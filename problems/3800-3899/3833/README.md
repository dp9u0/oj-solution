...
[LeetCode Problem Description](https://leetcode.com/problems/count-dominant-indices/description/)
...
给定整数数组 nums，索引 i 称为"主导"的当且仅当 nums[i] > avg(nums[i+1..n-1])。最右边的元素不算。统计主导索引的个数。

## 解题思路

从右向左维护后缀和，对每个 i 判断 nums[i] > suffixSum / count。O(n)。
