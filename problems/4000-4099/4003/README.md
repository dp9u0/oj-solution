...
[LeetCode Problem Description](https://leetcode.com/problems/minimum-cost-path-with-alternating-directions-iii/description/)
...
m×n 网格，进入格子 (i,j) 花 `(i+1)(j+1)`。从 (0,0) 出发（先付其进门费），动作从 1 开始编号：奇数动作须向右/下，偶数动作须向左/上；违规移动付 目标进门费 + 出发格 penalty；原地等待付 penalty。每个动作后奇偶翻转。求到 (m−1, n−1) 的最小总花费。

示例 1：`2, 2, [[5,3],[1,4]]` → `8`

## 解题思路

状态 = (格子, 下一动作奇偶) 的 **Dijkstra**（花费非负）：转移为等待（penalty）与四方向移动（合规方向仅付进门费，违规方向加出发格 penalty），奇偶翻转。m·n ≤ 10^5 → 2×10^5 状态。到达目标格即返回。O(mn log mn)。