...
Heroes hold `gem[i]` energy gems. Operation `[x,y]`: hero x gives half (floor) of their gems to y, sequentially. After all, return `max - min` gem count.

**Example:** `[3,1,2]`, ops `[[0,2],[2,1],[2,0]]` → 2.

**Constraints:** gem ≤ 10^3 entries, ops ≤ 10^4.

---

## Approach

Simulate each operation: `give = floor(gem[x]/2)`, subtract from x, add to y. Finally compute max−min.

Complexity: `O(len + ops)`.
